    ...
    if let Some(ser) = check_content_type(req)
        ...
        .and_then(|base_set| get_db(req)
            .and_then(|conn| get_school_id(req)
                .and_then(|school_id| base_set.save(None, school_id, conn))))
        ...
        .and_then(|base_sets| get_db(req)
            .and_then(|conn| get_school_id(req)
                .map(|school_id| base_sets
                    .into_iter()
                    .filter_map(|base_set| base_set.save(None, school_id, conn))
                    .collect::<Vec<BaseSet>>())))
...
pub fn delete(req: &mut Request) -> IronResult<Response> {
    if extract_id(req)
        .and_then(|id| get_db(req)
            .and_then(|conn| get_school_id(req)
                .and_then(|school_id| BaseSet::delete(id, school_id, conn)))).is_some() {
